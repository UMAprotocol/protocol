...
import { getFlags } from "../common/utils";
...
const transactionRequest = {
  transactionHash: "0xb0e367b8e7f8bd5ebbbd2056a01d57667c94967b0d6f1115541fd52caf2e161f",
  chainId: 1,
};

describe("OptimisticOracleClient", function () {
  let client: Client;
  let store: Store;
  beforeAll(function () {
  ...
  test("flags", function () {
    const result = getFlags(store.read());
    assert.ok(result);
  ...
  test("setActiveRequestByTransaction2", async function (done) {
    const id = client.setActiveRequestByTransaction(transactionRequest);
