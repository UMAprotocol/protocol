...
describe("VaultPriceFeed.js", function () {
  ...
  it("Basic current price", async function () {
    await vaultMock.methods.setPricePerFullShare(parseFixed("50", tokenDecimals)).send({ from: owner });
  ...
  it("Correctly selects most recent price", async function () {
    await vaultMock.methods.setPricePerFullShare(parseFixed("50", tokenDecimals)).send({ from: owner });
    await vaultMock.methods.setPricePerFullShare(parseFixed("100", tokenDecimals)).send({ from: owner });
    await vaultMock.methods.setPricePerFullShare(parseFixed("0.1", tokenDecimals)).send({ from: owner });
    await vaultPriceFeed.update();
  ...
  it("Historical Price", async function () {
    ...
    await vaultMock.methods.setPricePerFullShare(parseFixed("10", tokenDecimals)).send({ from: owner });
