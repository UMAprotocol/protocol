...
class PerlinXPriceFeed extends PriceFeedInterface {
   ...
   */
    ...
    async update() {
        ...
        const priceUrl = `https://marketdata.tradermade.com/api/v1/minute_historical?currency=${this.pair}&date_time=${_getFormattedDate(currentTime)}&api_key=${this.tradermadeApiKey}`
        const historicalPricesUrl = `https://marketdata.tradermade.com/api/v1/timeseries?currency=${this.pair}&api_key=${this.tradermadeApiKey}&start_date=${_getFormattedDate(currentTime - this.lookback)}&end_date=${_getFormattedDate(currentTime)}&format=records&interval=minute&period=${this.periods}`
        ...
        // 3. Check responses.
        if (!priceResponse || priceResponse.close || typeof priceResponse.close !== "string") {
            throw new Error(`🚨Could not parse price result from url ${priceUrl}: ${JSON.stringify(priceResponse)}`);
        }

        if (
            !historyResponse ||
            !historyResponse.quotes ||
            !Array.isArray(historyResponse.quotes)
        ) {
        ...
        //       ...
        // }
        const newHistoricalPricePeriods = historyResponse.quotes
            .map(row => ({
    ...
    _getFormattedDate(timestamp) {
        ...
        date = date.setMinutes(date.getMinutes() - 1); // Subtract one minute
        return date.toISOString().slice(0, 16).replace('Z', '').replace('T', '-');
    }
