...
class HarvestVaultPriceFeed extends PriceFeedInterface {
  /**
   ...
   * @param {Object} harvestVaultAbi Harvest Vault abi object to create a contract instance.
  ...
  constructor({
    ...
    assert(harvestVaultAbi, "harvestVaultAbi required");
    ...
    this.vault = new web3.eth.Contract(harvestVaultAbi, vaultAddress);
  ...
  async _convertDecimals(value) {
    if (!this.cachedConvertDecimalsFn) {
      const underlyingTokenAddress = await this.vault.methods.underlying().call();
