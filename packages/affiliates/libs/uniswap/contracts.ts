...
export function getAddress(network: NetworkName, contractName: ContractType) {
  ...
  // typescript complains that addresses object could be undefined even though the network type and Map is defined.
  assert(addresses, "invalid network: " + network);
...
export function PoolClient(provider: ethers.providers.Provider) {
  ...
  async function getPoolState(params: { blockNumber: number | string | undefined; address: string }) {
  ...
  async function getPositionState(params: {
    position: Position;
