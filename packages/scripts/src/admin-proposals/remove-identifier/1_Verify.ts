// This script verify that the indentifier has been deprecated in the IdentifierWhitelist contract in mainnet.
...
async function main() {
  ...
  console.log(` 1. Validating identifier ${process.env.IDENTIFIER} is not supported`);
  assert(!(await identifierWhitelist.isIdentifierSupported(newIdentifier)));
  console.log(`✅ ${process.env.IDENTIFIER} identifier is not supported`);
}
