...
let currentVersionBeingTested;
let runTestForContractVersion = function(supportedVersions) {
  ...
  // if the test supports "any" then it should not be skipped.
  if (supportedVersions == "any") return true;
  // if the version being tested is not one of the supported Versions then return true to skip.
  return supportedVersions.includes(currentVersionBeingTested);
};
...
contract("Liquidator.js", function(accounts) {
  ...
  // let const financialContractInstance;
  ...
  SUPPORTED_CONTRACT_VERSIONS.forEach(function(contractVersion) {
    currentVersionBeingTested = contractVersion;
    const currentTypeBeingTested = contractVersion.substring(0, contractVersion.indexOf("-"));
    const contractVersionBeingTested = contractVersion.substring(
      contractVersion.indexOf("-") + 1,
    ...
    for (let testConfig of configs) {
      describe(`${testConfig.collateralDecimals} collateral, ${testConfig.syntheticDecimals} synthetic & ${testConfig.priceFeedDecimals} pricefeed decimals, on for smart contract version ${contractVersion}`, function() {
        ...
        const financialContractInstance = getTruffleContract(currentTypeBeingTested, web3, contractVersionBeingTested);
        const Finder = getTruffleContract("Finder", web3, contractVersionBeingTested);
        const IdentifierWhitelist = getTruffleContract("IdentifierWhitelist", web3, contractVersionBeingTested);
        const AddressWhitelist = getTruffleContract("AddressWhitelist", web3, contractVersionBeingTested);
        const MockOracle = getTruffleContract("MockOracle", web3, contractVersionBeingTested);
        const Token = getTruffleContract("ExpandedERC20", web3, contractVersionBeingTested);
        const SyntheticToken = getTruffleContract("SyntheticToken", web3, contractVersionBeingTested);
        const Timer = getTruffleContract("Timer", web3, contractVersionBeingTested);
        const Store = getTruffleContract("Store", web3, contractVersionBeingTested);
        ...
        beforeEach(async function() {
          ...
          const constructorParams = await _createConstructorParamsForContractVersion(
            contractVersionBeingTested,
            currentTypeBeingTested
          );
          ...
          if (currentTypeBeingTested == "Perpetual") await emp.applyFundingRate();
          ...
          liquidatorConfig = {
            crThreshold: 0,
            contractType: currentTypeBeingTested,
            contractVersion: contractVersionBeingTested
          };
        ...
        (runTestForContractVersion(["any"]) ? it.only : () => {})(
        ...
        (runTestForContractVersion(["any"]) ? it : () => {})(
        ...
        describe("Overrides the default liquidator configuration settings", function() {
          (runTestForContractVersion(["any"]) ? it : () => {})("Cannot set `crThreshold` >= 1", async function() {
            let errorThrown;
            try {
              liquidatorConfig = {
                ...liquidatorConfig,
          ...
          (runTestForContractVersion(["any"]) ? it : () => {})("Cannot set `crThreshold` < 0", async function() {
            let errorThrown;
            try {
              liquidatorConfig = {
                ...liquidatorConfig,
          ...
          (runTestForContractVersion(["any"]) ? it : () => {})("Sets `crThreshold` to 2%", async function() {
            liquidatorConfig = {
              ...liquidatorConfig,
              crThreshold: 0.02
            };
        ...
        describe("enabling withdraw defense feature", () => {
          (runTestForContractVersion(["any"]) ? it : () => {})("should initialize when enabled", async () => {
            liquidatorConfig = {
              ...liquidatorConfig,
        ...
        describe("Liquidator correctly deals with funding rates from perpetual contract", () => {
          (runTestForContractVersion(["Perpetual-latest"]) ? it : () => {})(
            "Can correctly detect invalid withdrawals and liquidate them",
            async function() {
              // sponsor1 creates a position with 125 units of collateral, creating 100 synthetic tokens.
              await emp.create(
