...
- `SETTLE_ONLY_DISPUTED`: When `true`, standard `OptimisticOracleV2` only settles resolved disputes (`false` by default). Ignored for every other oracle type.
- `SETTLE_INCLUDE_LIST`: JSON array of `"<txHash>:<logIndex>"` proposal identifiers, where `logIndex` is the block-level index of the `ProposePrice` log (for example, ethers' `event.logIndex`), not its position within the transaction. For standard OOv2 the bot settles **only** these proposals. Managed OOv2 fetches and processes only the listed proposals, regardless of dispute status, and defaults to an empty list that settles nothing. OOv2 contract types only — setting it for another `ORACLE_TYPE` throws at startup. Example: `["0xabc...def:5"]`.
