...
spoke.post("/", async (req, res) => {
  try {
    ...
    logger.debug({
      ...
      childProcessIdentifier: _getChildProcessIdentifier(req),
      execResponse: JSON.stringify(execResponse)
    });
    ...
    logger.debug({
      ...
      childProcessIdentifier: _getChildProcessIdentifier(req),
      jsonBody: req.body,
      execResponse: execResponse instanceof Error ? execResponse.message : JSON.stringify(execResponse)
    });
    await delay(2); // Wait a few seconds to be sure the the winston logs are processed upstream.
    res.status(500).send({
      message: "Process exited with error",
...
function _stripExecStdout(output) {
  ...
  try {
    ...
    let keyValuedLogs = {};
    logsArray.forEach(logMessage => {
      keyValuedLogs[logMessage["message"]] = logMessage;
    });
    return keyValuedLogs;
  } catch (error) {
