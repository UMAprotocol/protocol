...
/**
 * @title Optimistic Mock Oracle.
 * @notice Mock pre-DVM escalation contract that allows faster settlement.
 * @dev The primary difference between this and the real OptimisticOracle is
 * the use of the MockOracle as the stand-in for the Voting contract. Since the
 * MockOracle does not use ancillary data, the stampAncillaryData function was
 * also removed. This lets us test an example contract interacting with the
 * Optimistic Oracle while pushing prices to the MockOracle to test disputes.
 */
contract OptimisticOracle is OptimisticOracleInterface, Testable, Lockable {
    ...
    function getState(
        ...
        return
            _getOracle().hasPrice(identifier, timestamp)
                ? State.Resolved
                : State.Disputed;
    }
