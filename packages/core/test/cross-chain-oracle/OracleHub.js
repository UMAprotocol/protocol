...
describe("OracleHub.js", async () => {
  ...
  it("publishPrice", async function () {
    ...
    // Works even if the price has not been requested on the hub via _requestPrice.
    ...
    // Calling publishPrice again for same price request will not revert but will not send another external call
    // to the Messenger.
    await publishPrice.send({ from: owner });
