...
describe("BridgePool", () => {
  ...
  describe("Speed up relay", () => {
    it("Can add instant relayer to pending relay", async () => {
      ...
      await optimisticOracle.methods
        ...
        .send({ from: disputer });
      assert(await didContractThrow(bridgePool.methods.speedUpRelay(depositHash).call({ from: instantRelayer })));
  ...
  describe("Dispute pending relay", () => {
    ...
    it("Instant relayer address persists for subsequent relays after a pending relay is disputed", async () => {
      ...
      // Speed up relay.
      await bridgePool.methods.speedUpRelay(depositHash).send({ from: instantRelayer });
