...
// Contracts to test
const PerpetualPositionManagerPoolParty = artifacts.require("PerpetualPositionManagerPoolParty");
...
contract("PerpetualPositionManagerPoolParty", function(accounts) {
  ...
  const priceFeedIdentifier = web3.utils.padRight(utf8ToHex("TEST_IDENTIIFER"), 64);
  ...
  const checkBalances = async (expectedSponsorTokens, expectedSponsorCollateral) => {
    ...
    assert.equal((await tokenCurrency.balanceOf.call(sponsor)).toString(), expectedSponsorTokens.toString());

    assert.equal((await positionManager.totalPositionCollateral.call()).toString(), expectedTotalCollateral.toString());

    assert.equal(
      (await positionManager.globalPositionData.call()).totalTokensOutstanding.toString(),
      expectedTotalTokens.toString()
    );
    assert.equal(await collateral.balanceOf.call(positionManager.address), expectedTotalCollateral.toString());
  };
  ...
  const expectNoExcessCollateralToTrim = async () => {
    ...
    let beneficiaryCollateralBalance = await collateral.balanceOf.call(beneficiary);
  ...
  beforeEach(async function() {
    ...
    collateral = await MarginToken.new("Collateral Test Token", "TEST", 18, { from: collateralOwner });
    ...
    if (FeePayerPoolPartyLib.setAsDeployed || PerpetualPositionManagerPoolPartyLib.setAsDeployed) {
      try {
        await PerpetualPositionManagerPoolParty.link(feePayerPoolPartyLib);
        await PerpetualPositionManagerPoolParty.link(perpetualPositionManagerPoolPartyLib);
      } catch (e) {
        // Allow this to fail in the Buidler case.
      }
    } else {
      // Truffle
      await PerpetualPositionManagerPoolParty.link(FeePayerPoolPartyLib, feePayerPoolPartyLib.address);
    ...
    positionManager = await PerpetualPositionManagerPoolParty.new(positionManagerData, roles, {
      from: contractDeployer
    });
  ...
  it("Correct deployment and variable assignment", async function() {
    ...
    assert.equal((await positionManager.positionManagerData.call()).emergencyShutdownTimestamp, 0);
  ...
  it("Valid constructor params", async function() {
    ...
    newPositionManagerData.priceFeedIdentifier = web3.utils.padRight(utf8ToHex("UNREGISTERED"), 64);
    // Pricefeed identifier must be whitelisted.
    assert(
      await didContractThrow(
        PerpetualPositionManagerPoolParty.new(newPositionManagerData, roles, { from: contractDeployer })
      )
    );
  });
  ...
  it("Withdrawal liveness overflow", async function() {
    ...
    positionManager = await PerpetualPositionManagerPoolParty.new(newPositionManagerData, roles, {
      from: contractDeployer
    });
    await tokenCurrency.addMinter(positionManager.address);
    await tokenCurrency.addBurner(positionManager.address);
  ...
  it("Withdrawal request", async function() {
    ...
    assert(await didContractThrow(positionManager.requestWithdrawal({ rawValue: toWei("0.001") }, { from: other })));
    ...
    await positionManager.setCurrentTime((await positionManager.getCurrentTime.call()).toNumber() + withdrawalLiveness);
  ...
  it("Non sponsor can use depositTo", async function() {
    ...
    assert.equal((await positionManager.getCollateral.call(sponsor)).toString(), toWei("2"));
    assert.equal((await positionManager.getCollateral.call(other)).toString(), "0");
    await positionManager.renouncePool({ from: other });
  });
  ...
  it("Sponsor can use repay to decrease their debt", async function() {
    ...
    const initialSponsorTokenDebt = toBN((await positionManager.positions.call(sponsor)).tokensOutstanding.rawValue);
    const initialTotalTokensOutstanding = toBN(
      (await positionManager.globalPositionData.call()).totalTokensOutstanding.rawValue
    );
    ...
    const tokenDebtDecreased = initialSponsorTokenDebt.sub(
      toBN((await positionManager.positions.call(sponsor)).tokensOutstanding.rawValue)
    );
    const totalTokensOutstandingDecreased = initialTotalTokensOutstanding.sub(
      toBN((await positionManager.globalPositionData.call()).totalTokensOutstanding.rawValue)
    );
    ...
    assert.equal((await positionManager.positions.call(sponsor)).tokensOutstanding.rawValue, toWei("60"));
    ...
    assert.equal((await positionManager.positions.call(sponsor)).tokensOutstanding.rawValue, minSponsorTokens);
  ...
  it("Emergency shutdown: lifecycle", async function() {
    ...
    assert(await didContractThrow(positionManager.emergencyShutdown({ from: sponsor })));
  ...
  describe("Precision loss as a result of regular fees is handled as expected", () => {
    ...
    it("Fee multiplier is set properly with precision loss, and fees are paid as expected", async () => {
      ...
      assert.equal(
        (await positionManager.feePayerData.call()).cumulativeFeeMultiplier.toString(),
      ...
      assert.equal((await positionManager.totalPositionCollateral.call()).toString(), "28");
      assert.equal(
        (await positionManager.globalPositionData.call()).rawTotalPositionCollateral.rawValue.toString(),
        "30"
      );
    ...
    it("settleEmergencyShutdown() returns the same amount of collateral that totalPositionCollateral is decreased by", async () => {
      ...
      const tokenHolderInitialCollateral = await collateral.balanceOf.call(tokenHolder);
      await positionManager.addPool(tokenHolder, { from: sponsor });
      await positionManager.settleEmergencyShutdown({ from: tokenHolder });
      const tokenHolderFinalCollateral = await collateral.balanceOf.call(tokenHolder);
      const tokenHolderFinalSynthetic = await tokenCurrency.balanceOf.call(tokenHolder);
      ...
      assert.equal(tokenHolderFinalCollateral.sub(tokenHolderInitialCollateral), expectedTokenHolderFinalCollateral);
      assert.equal((await collateral.balanceOf.call(positionManager.address)).toString(), "18");
      assert.equal((await positionManager.totalPositionCollateral.call()).toString(), "17");
      assert.equal(
        (await positionManager.globalPositionData.call()).rawTotalPositionCollateral.rawValue.toString(),
        "18"
      );
      ...
      // (30 - 1 - 11 - 16 = 2)
      assert.equal((await collateral.balanceOf.call(positionManager.address)).toString(), "2");
      assert.equal((await positionManager.totalPositionCollateral.call()).toString(), "1");
    ...
    it("withdraw() returns the same amount of collateral that totalPositionCollateral is decreased by", async () => {
      ...
      assert.equal((await positionManager.globalPositionData.call()).rawTotalPositionCollateral.toString(), "18");
    ...
    it("redeem() returns the same amount of collateral that totalPositionCollateral is decreased by", async () => {
      ...
      assert.equal((await tokenCurrency.balanceOf.call(sponsor)).toString(), "11");
  ...
  it("Oracle swap post shutdown", async function() {
    ...
    await positionManager.addPool(tokenHolder, { from: sponsor });
    await positionManager.settleEmergencyShutdown({ from: tokenHolder });
    let collateralPaid = (await collateral.balanceOf.call(tokenHolder)).sub(initialCollateral);
    ...
    const mockOracleInterfaceName = web3.utils.padRight(utf8ToHex(interfaceName.Oracle), 64);
    ...
    await positionManager.settleEmergencyShutdown({ from: sponsor });
    collateralPaid = (await collateral.balanceOf.call(sponsor)).sub(initialCollateral);
    ...
    collateralPaid = (await collateral.balanceOf.call(other)).sub(initialCollateral);
    assert.equal(collateralPaid, toWei("120"));
    await positionManager.renouncePool({ from: tokenHolder });
    await positionManager.renouncePool({ from: other });
  });
  ...
  it("Oracle price can resolve less than 0", async function() {
    ...
    // each token debt is worth 0 and the sponsor should get back their full collateral, even though they dont have all
    // the tokens. The token holder should get nothing.
    await mockOracle.pushPrice(priceFeedIdentifier, emergencyShutdownTime, toWei("-0.1"));
  ...
  it("Undercapitalized contract", async function() {
    ...
    assert.equal((await collateral.balanceOf.call(tokenHolder)).toString(), startingBalance.add(toBN(toWei("150"))));
    ...
    await positionManager.settleEmergencyShutdown({ from: other });
    assert.equal((await collateral.balanceOf.call(other)).toString(), startingBalance.add(toBN(toWei("50"))));
    ...
    await positionManager.settleEmergencyShutdown({ from: sponsor });
    assert.equal((await collateral.balanceOf.call(sponsor)).toString(), startingBalance.add(toBN("0")));
    await positionManager.renouncePool({ from: tokenHolder });
    await positionManager.renouncePool({ from: other });
  });
  ...
  it("Can withdraw excess collateral", async function() {
    ...
    let beneficiaryTokenBalance = await tokenCurrency.balanceOf.call(beneficiary);
  ...
  it("Non-standard ERC20 delimitation", async function() {
    ...
    const nonStandardToken = await SyntheticToken.new(syntheticName, syntheticSymbol, 18);
    ...
    assert.equal(
      (await USDCToken.balanceOf.call(custompositionManager.address)).toString(),
      expectedContractCollateral.toString()
    );
    assert.equal((await tokenCurrency.balanceOf.call(sponsor)).toString(), expectedSponsorTokens.toString());
    assert.equal(
    ...
    assert.equal(
      (await custompositionManager.positions.call(sponsor)).tokensOutstanding.toString(),
    ...
    assert.equal(
      (await custompositionManager.globalPositionData.call()).totalTokensOutstanding.rawValue.toString(),
      expectedSponsorTokens.toString()
    );
    ...
    const tokenHolderInitialCollateral = await USDCToken.balanceOf.call(tokenHolder);
    const tokenHolderInitialSynthetic = await tokenCurrency.balanceOf.call(tokenHolder);
    assert.equal(tokenHolderInitialSynthetic, tokenHolderTokens);
    ...
    const sponsorsPosition = await custompositionManager.positions.call(sponsor);
    assert.equal(sponsorsPosition.rawCollateral.rawValue, 0);
    assert.equal(sponsorsPosition.tokensOutstanding.rawValue, 0);
    assert.equal(sponsorsPosition.withdrawalRequestPassTimestamp.toString(), 0);
    assert.equal(sponsorsPosition.withdrawalRequestAmount.rawValue, 0);
  ...
  it("Can not delete a sponsor position externally", async () => {
    await collateral.approve(positionManager.address, initialPositionCollateral, { from: sponsor });
    await positionManager.create(
      { rawValue: initialPositionCollateral.toString() },
      { rawValue: initialPositionTokens.toString() },
      { from: sponsor }
    );
    assert(await didContractThrow(positionManager.deleteSponsorPosition(sponsor, { from: other })));
  });
  ...
  it("Check position manager roles", async () => {
    ...
    assert(await didContractThrow(positionManager.redeem({ rawValue: redeemAmount.toString() }, { from: other })));
    // Check Admin role
    ...
    assert(await didContractThrow(positionManager.addPool(other, { from: other })));
  ...
  it("Check position manager roles", async () => {
    ...
    assert.equal(await tokenCurrency.isAdmin.call(positionManager.address), false);
    await tokenCurrency.addAdmin(positionManager.address, { from: contractDeployer });
    assert.equal(await tokenCurrency.isAdmin.call(positionManager.address), true);
    ...
    // Check add all roles
    await positionManager.addSyntheticTokenAdminAndMinterAndBurner(tokenHolder, { from: sponsor });
    ...
    await positionManager.renounceSyntheticTokenMinter({ from: sponsor });
    assert.equal(await tokenCurrency.isMinter.call(positionManager.address), false);
    await positionManager.renounceSyntheticTokenBurner({ from: sponsor });
    assert.equal(await tokenCurrency.isBurner.call(positionManager.address), false);
    await positionManager.renounceSyntheticTokenAdmin({ from: sponsor });
    assert.equal(await tokenCurrency.isAdmin.call(positionManager.address), false);
    // Check renounce all roles
    await tokenCurrency.addAdmin(positionManager.address, { from: contractDeployer });
    await tokenCurrency.addMinter(positionManager.address, { from: contractDeployer });
    await tokenCurrency.addBurner(positionManager.address, { from: contractDeployer });
    assert.equal(await tokenCurrency.isAdmin.call(positionManager.address), true);
    assert.equal(await tokenCurrency.isMinter.call(positionManager.address), true);
    assert.equal(await tokenCurrency.isBurner.call(positionManager.address), true);
    await positionManager.renounceSyntheticTokenAdminAndMinterAndBurner({ from: sponsor });
    assert.equal(await tokenCurrency.isAdmin.call(positionManager.address), false);
    assert.equal(await tokenCurrency.isMinter.call(positionManager.address), false);
    assert.equal(await tokenCurrency.isBurner.call(positionManager.address), false);
  });
