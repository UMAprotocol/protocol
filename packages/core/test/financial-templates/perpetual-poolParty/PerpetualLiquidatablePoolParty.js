...
const LiquidatablePoolParty = artifacts.require("PerpetualLiquidatablePoolParty");
...
contract("PerpetualLiquidatablePoolParty", function(accounts) {
  ...
  beforeEach(async () => {
    ...
    collateralToken = await Token.new("Test Collateral Token", "TEST_COLL", 18, { from: contractDeployer });
    ...
    if (
      ...
      // Truffle
      await LiquidatablePoolParty.link(FeePayerPoolPartyLib, feePayerPoolPartyLib.address);
      await LiquidatablePoolParty.link(
        PerpetualPositionManagerPoolPartyLib,
        perpetualPositionManagerPoolPartyLib.address
      );
      await LiquidatablePoolParty.link(PerpetualLiquidatablePoolPartyLib, perpetualLiquidatablePoolPartyLib.address);
    }
    ...
    await collateralToken.addMinter(contractDeployer, { from: contractDeployer });
  ...
  const expectNoExcessCollateralToTrim = async () => {
    ...
    let beneficiaryCollateralBalance = await collateralToken.balanceOf.call(beneficiary);
  ...
  describe("Creating a liquidation on a valid position", () => {
    ...
    it("Liquidator pays final fee", async () => {
      ...
      assert.equal(
        intitialBalance.sub(await collateralToken.balanceOf.call(liquidator)).toString(),
    ...
    it("Partial liquidation", async () => {
      ...
      let liquidation = await liquidationContract.liquidations.call(sponsor, liquidationId);
      ...
      liquidation = await liquidationContract.liquidations.call(sponsor, liquidationId);
      assert.equal(expectedLiquidatedTokens.toString(), liquidation.tokensOutstanding.toString());
  ...
  describe("Full liquidation has been created", () => {
    ...
    describe("Get a Liquidation", () => {
      it("Liquidator burned synthetic tokens", async () => {
        assert.equal((await syntheticToken.balanceOf.call(liquidator)).toString(), "0");
      ...
      it("Liquidation decrease underlying token debt and collateral", async () => {
        ...
        const totalTokensOutstandingAfter = (await liquidationContract.globalPositionData.call()).totalTokensOutstanding
          .rawValue;
        assert.equal(totalTokensOutstandingAfter, 0);
      });
      it("Liquidation exists and params are set properly", async () => {
        const newLiquidation = await liquidationContract.liquidations.call(sponsor, liquidationParams.liquidationId);
    ...
    describe("Dispute a Liquidation", () => {
      ...
      it("Request to dispute succeeds and Liquidation params changed correctly", async () => {
        ...
        assert.equal((await collateralToken.balanceOf.call(disputer)).toString(), "0");
        const liquidation = await liquidationContract.liquidations.call(sponsor, liquidationParams.liquidationId);
      ...
      it("Dispute pays a final fee", async () => {
        ...
        const storeInitialBalance = toBN(await collateralToken.balanceOf.call(store.address));
        await liquidationContract.dispute(liquidationParams.liquidationId, sponsor, { from: disputer });
        const storeAfterDisputeBalance = toBN(await collateralToken.balanceOf.call(store.address));
        ...
        const expectedContractBalance = toBN(amountOfCollateral)
          ...
          .add(finalFeeAmount);
        assert.equal(
          (await collateralToken.balanceOf.call(liquidationContract.address)).toString(),
    ...
    describe("Withdraw: Liquidation expires without dispute (but synthetic token has not expired)", () => {
      ...
      it("Liquidation does not exist", async () => {
        assert(
          await didContractThrow(liquidationContract.withdrawLiquidation(liquidationParams.falseLiquidationId, sponsor))
        );
      });
    ...
    describe("Withdraw: Liquidation dispute resolves", () => {
      ...
      describe("Dispute succeeded", () => {
        ...
        it("Liquidated contact should have no assets remaining after all withdrawals and be deleted", async () => {
          await liquidationContract.withdrawLiquidation(liquidationParams.liquidationId, sponsor);
          assert.equal((await collateralToken.balanceOf.call(liquidationContract.address)).toString(), "0");
        ...
        it("Fees on liquidation", async () => {
          ...
          let startBalanceLiquidator = await collateralToken.balanceOf.call(liquidator);
          let startBalanceDisputer = await collateralToken.balanceOf.call(disputer);
  ...
  describe("Weird Edge cases", () => {
    ...
    it("Dispute rewards should not add to over 100% of TRV", async () => {
      ...
      invalidConstructorParameter.liquidatableParams.sponsorDisputeRewardPct = { rawValue: toWei("0.6") };
      invalidConstructorParameter.liquidatableParams.disputerDisputeRewardPct = { rawValue: toWei("0.5") };
      assert(
        await didContractThrow(LiquidatablePoolParty.new(invalidConstructorParameter, { from: contractDeployer }))
      );
    });
    it("Collateral requirement should be later than 100%", async () => {
      let invalidConstructorParameter = constructorParams;
      invalidConstructorParameter.liquidatableParams.collateralRequirement = { rawValue: toWei("0.95") };
      assert(
        await didContractThrow(LiquidatablePoolParty.new(invalidConstructorParameter, { from: contractDeployer }))
      );
    });
    it("Dispute bond can be over 100%", async () => {
      ...
      newTokenconstructorParams.positionManagerParams.tokenAddress = syntheticToken.address;
      const edgeLiquidationContract = await LiquidatablePoolParty.new(newTokenconstructorParams, {
        from: contractDeployer
      });
  ...
  describe("Non-standard ERC20 delimitation", () => {
    ...
    const USDCDisputerDisputeReward = disputerDisputeRewardPct.mul(USDCSettlementTRV).div(toBN(toWei("1"))); // 5e6
    ...
    beforeEach(async () => {
      ...
      USDCConstructorParameters.positionManagerParams.collateralAddress = collateralToken.address;
      USDCConstructorParameters.positionManagerParams.tokenAddress = syntheticToken.address;
      USDCConstructorParameters.positionManagerParams.minSponsorTokens = {
        rawValue: minSponsorTokens.div(USDCScalingFactor).toString()
      };
      USDCLiquidationContract = await LiquidatablePoolParty.new(USDCConstructorParameters, {
        from: contractDeployer
      });
      ...
      await store.setFinalFee(collateralToken.address, { rawValue: USDCFinaleFee.toString() });
    ...
    describe("Dispute succeeded", () => {
      ...
      it("Rewards are distributed", async () => {
        const sponsorUSDCBalanceBefore = await collateralToken.balanceOf.call(sponsor);
        const disputerUSDCBalanceBefore = await collateralToken.balanceOf.call(disputer);
        const liquidatorUSDCBalanceBefore = await collateralToken.balanceOf.call(liquidator);
        await USDCLiquidationContract.withdrawLiquidation(liquidationParams.liquidationId, sponsor);
        const sponsorUSDCBalanceAfter = await collateralToken.balanceOf.call(sponsor);
        const disputerUSDCBalanceAfter = await collateralToken.balanceOf.call(disputer);
        const liquidatorUSDCBalanceAfter = await collateralToken.balanceOf.call(liquidator);
        ...
        // Expected Liquidator payment => TRV - dispute reward - sponsor reward
        const expectedPaymentLiquidator = USDCSettlementTRV.sub(USDCDisputerDisputeReward).sub(
        ...
        // Expected Disputer payment => disputer reward + dispute bond
        const expectedPaymentDisputer = USDCDisputerDisputeReward.add(USDCDisputeBond).add(USDCFinaleFee);
        assert.equal(
        ...
        assert.equal((await collateralToken.balanceOf.call(USDCLiquidationContract.address)).toString(), "0");
    ...
    describe("Dispute failed", () => {
      ...
      it("Rewards liquidator only, liquidation is deleted", async () => {
        ...
        const liquidatorUSDCBalanceAfter = await collateralToken.balanceOf.call(liquidator);
        // Expected Liquidator payment => lockedCollateral + liquidation.disputeBond % of liquidation.lockedCollateral to liquidator
        const expectedPayment = USDCAmountOfCollateral.add(USDCDisputeBond).add(USDCFinaleFee);
        assert.equal(
          liquidatorUSDCBalanceAfter.sub(liquidatorUSDCBalanceBefore).toString(),
  ...
  describe("Precision loss is handled as expected", () => {
    beforeEach(async () => {
      ...
      const newTokenconstructorParams = constructorParams;
      newTokenconstructorParams.positionManagerParams.tokenAddress = syntheticToken.address;
      newTokenconstructorParams.positionManagerParams.minSponsorTokens = { rawValue: "0" };
      liquidationContract = await LiquidatablePoolParty.new(newTokenconstructorParams, { from: contractDeployer });
      await syntheticToken.addMinter(liquidationContract.address);
      await syntheticToken.addBurner(liquidationContract.address);
    ...
    it("Fee multiplier is set properly with precision loss, and fees are paid as expected.", async () => {
      ...
      let collateralAmount = await liquidationContract.getCollateral.call(sponsor);
      assert(toBN(collateralAmount.rawValue).eq(toBN("0")));
      assert.equal(
        (await liquidationContract.feePayerData.call()).cumulativeFeeMultiplier.toString(),
      ...
      assert.equal((await collateralToken.balanceOf.call(liquidationContract.address)).toString(), "29");
      assert.equal((await liquidationContract.liquidatableData.call()).rawLiquidationCollateral.toString(), "28");
      assert.equal((await liquidationContract.globalPositionData.call()).rawTotalPositionCollateral.toString(), "0");
    ...
    it("Liquidation object is set up properly", async () => {
      let liquidationData = await liquidationContract.liquidations.call(sponsor, 0);
    ...
    it("withdrawLiquidation() returns the same amount of collateral that liquidationCollateral is decreased by", async () => {
      ...
      assert.equal((await collateralToken.balanceOf.call(liquidator)).toString(), "27");
      assert.equal((await collateralToken.balanceOf.call(liquidationContract.address)).toString(), "2");
      let deletedLiquidationData = await liquidationContract.liquidations.call(sponsor, 0);
      ...
      assert.equal((await liquidationContract.liquidatableData.call()).rawLiquidationCollateral.toString(), "1");
