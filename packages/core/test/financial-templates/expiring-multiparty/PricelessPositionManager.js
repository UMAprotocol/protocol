...
contract("PricelessPositionManager", function(accounts) {
  ...
  const checkBalances = async (expectedSponsorTokens, expectedSponsorCollateral) => {
    ...
    const sponsorCollateral = await pricelessPositionManager.getCollateral.call(sponsor);
    ...
    assert.equal(
      (await pricelessPositionManager.totalPositionCollateral.call()).toString(),
  ...
  it("Transfer position request", async function() {
    ...
    assert.equal((await pricelessPositionManager.getCollateral.call(sponsor)).toString(), amountCollateral);
    assert.equal(
    ...
    assert.equal((await pricelessPositionManager.getCollateral.call(tokenHolder)).toString(), amountCollateral);
  ...
  it("Non sponsor can use depositTo", async function() {
    ...
    assert.equal((await pricelessPositionManager.getCollateral.call(sponsor)).toString(), toWei("2"));
    assert.equal((await pricelessPositionManager.getCollateral.call(other)).toString(), "0");
  });
  ...
  it("Existing sponsor can use depositTo on other account", async function() {
    ...
    assert.equal((await pricelessPositionManager.getCollateral.call(sponsor)).toString(), toWei("2"));
    assert.equal((await pricelessPositionManager.getCollateral.call(other)).toString(), toWei("1"));
  });
  ...
  it("Basic fees", async function() {
    ...
    assert.equal((await pricelessPositionManager.getCollateral.call(sponsor)).toString(), toWei("0.99"));
    ...
    assert.equal((await pricelessPositionManager.totalPositionCollateral.call()).toString(), toWei("1.98"));
    ...
    let collateralAmount = await pricelessPositionManager.getCollateral.call(sponsor);
    ...
    collateralAmount = await pricelessPositionManager.getCollateral.call(sponsor);
    ...
    assert.equal((await pricelessPositionManager.getCollateral.call(sponsor)).toString(), toWei("98.99"));
    ...
    assert.equal((await pricelessPositionManager.getCollateral.call(sponsor)).toString(), "0");
  ...
  it("Gulps non-PfC collateral into PfC", async function() {
    ...
    await pricelessPositionManager.gulp();
    assert.equal((await pricelessPositionManager.getCollateral.call(other)).toString(), toWei("3.375"));
    assert.equal((await pricelessPositionManager.getCollateral.call(sponsor)).toString(), toWei("1.125"));
  ...
  describe("Precision loss is handled as expected", () => {
    ...
    it("Fee multiplier is set properly with precision loss, and fees are paid as expected", async () => {
      ...
      assert.equal((await pricelessPositionManager.totalPositionCollateral.call()).toString(), "28");
    ...
    it("settleExpired() returns the same amount of collateral that totalPositionCollateral is decreased by", async () => {
      ...
      assert.equal((await pricelessPositionManager.totalPositionCollateral.call()).toString(), "17");
      ...
      assert.equal((await pricelessPositionManager.totalPositionCollateral.call()).toString(), "1");
  ...
  it("Not enough collateral to pay final fees, reverts expire", async function() {
    ...
    let frozenCollateralAmount = await pricelessPositionManager.getCollateral.call(sponsor);
  ...
  it("Non-standard ERC20 delimitation", async function() {
    ...
    assert.equal((await tokenCurrency.balanceOf(sponsor)).toString(), expectedSponsorTokens.toString());
    assert.equal(
      (await customPricelessPositionManager.getCollateral.call(sponsor)).toString(),
      expectedContractCollateral.toString()
    );
    assert.equal(
    ...
    assert.equal(
      (await customPricelessPositionManager.totalPositionCollateral.call()).toString(),
      expectedContractCollateral.toString()
    );
    assert.equal(
