...
contract("PricelessPositionManager", function (accounts) {
  ...
  const proposeAndSettleOptimisticOraclePrice = async (priceFeedIdentifier, requestTime, price) => {
    await collateral.approve(optimisticOracle.options.address, toWei("1000000"), { from: proposer });
    ...
    assert(
      await didContractThrow(
        pricelessPositionManager.methods
          .requestWithdrawal({ rawValue: initialSponsorCollateral }, { from: sponsor })
          .send({ from: accounts[0] })
      )
    );
    assert(
      await didContractThrow(
        pricelessPositionManager.methods.requestTransferPosition({ from: sponsor }).send({ from: accounts[0] })
      )
    );
  });
  ...
  it("Lifecycle", async function () {
    ...
    assert(
      await didContractThrow(
        pricelessPositionManager.methods
          .create({ rawValue: createCollateral }, { rawValue: createTokens }, { from: sponsor })
          .send({ from: accounts[0] })
      )
    );
    await collateral.methods
    ...
    let sponsorInitialBalance = await collateral.methods.balanceOf(sponsor).call();
    ...
    let sponsorFinalBalance = await collateral.methods.balanceOf(sponsor).call();
    ...
    const redeemedCollateral = await redeem.methods.call({ rawValue: redeemTokens }).call({ from: sponsor });
  ...
  it("Withdrawal request", async function () {
    ...
    await collateral.approve(pricelessPositionManager.options.address, toWei("100000"), { from: sponsor });
    await tokenCurrency.approve(pricelessPositionManager.options.address, toWei("100000"), { from: sponsor });
    ...
    assert(
      await didContractThrow(
        pricelessPositionManager.methods.withdrawPassedRequest({ from: sponsor }).send({ from: accounts[0] })
      )
    );
    ...
    assert(
      await didContractThrow(
        pricelessPositionManager.methods
          .create({ rawValue: toWei("1") }, { rawValue: toWei("1") }, { from: sponsor })
          .send({ from: accounts[0] })
      )
    );
    assert(
    ...
    const resultCancelWithdrawal = await pricelessPositionManager.methods.cancelWithdrawal({ from: sponsor }).call();
    ...
    await pricelessPositionManager.setCurrentTime(
      (await pricelessPositionManager.methods.getCurrentTime().call()).toNumber() + withdrawalLiveness
    );
    ...
    amountWithdrawn = await withdraw.methods.call({ rawValue: toWei("1") }).call({ from: sponsor });
    ...
    assert(
      await didContractThrow(
        pricelessPositionManager.methods.cancelWithdrawal({ from: sponsor }).send({ from: accounts[0] })
      )
    );
    ...
    resultWithdrawPassedRequest = await pricelessPositionManager.methods
      .withdrawPassedRequest({ from: sponsor })
      .call();
    truffleAssert.eventEmitted(resultWithdrawPassedRequest, "RequestWithdrawalExecuted", (ev) => {
  ...
  it("Global collateralization ratio checks", async function () {
    await collateral.approve(pricelessPositionManager.options.address, toWei("100000"), { from: sponsor });
    await collateral.approve(pricelessPositionManager.options.address, toWei("100000"), { from: other });
    ...
    assert(
      await didContractThrow(
        pricelessPositionManager.methods
          .create({ rawValue: toWei("0") }, { rawValue: toWei("6") }, { from: other })
          .send({ from: accounts[0] })
      )
    );
  ...
  it("Transfer position request", async function () {
    ...
    assert(
      await didContractThrow(
        pricelessPositionManager.methods.cancelTransferPosition({ from: sponsor }).send({ from: accounts[0] })
      )
    );

    // Request transfer. Check event is emitted.
    const resultRequest = await pricelessPositionManager.methods.requestTransferPosition({ from: sponsor }).call();
    ...
    const resultCancel = await pricelessPositionManager.methods.cancelTransferPosition({ from: sponsor }).call();
    truffleAssert.eventEmitted(resultCancel, "RequestTransferPositionCanceled", (ev) => {
      return ev.oldSponsor == sponsor;
    });

    // They can now request again.
    await pricelessPositionManager.methods.requestTransferPosition({ from: sponsor }).send({ from: accounts[0] });
    ...
    await pricelessPositionManager.methods.cancelWithdrawal({ from: sponsor }).send({ from: accounts[0] });
  ...
  it("Frozen when post expiry", async function () {
    ...
    assert(
      await didContractThrow(
        pricelessPositionManager.methods
          .create({ rawValue: amountCollateral }, { rawValue: numTokens }, { from: sponsor })
          .send({ from: accounts[0] })
      )
    );
    assert(
  ...
  it("Settlement post expiry", async function () {
    ...
    const expireResult = await pricelessPositionManager.methods.expire({ from: other }).call();
    assert.equal(
    ...
    assert(
      await didContractThrow(
        pricelessPositionManager.methods.settleExpired({ from: tokenHolder }).send({ from: accounts[0] })
      )
    );
    ...
    let settleExpiredResult = await pricelessPositionManager.methods.settleExpired({ from: tokenHolder }).call();
    assert.equal(
      await pricelessPositionManager.methods.contractState().call(),
  ...
  describe("Financial Product Library", () => {
    it("Custom price transformation", async function () {
      ...
      // To settle positions the DVM needs to be to be queried to get the price at the settlement time.
      await pricelessPositionManager.methods.expire({ from: other }).send({ from: accounts[0] });
  ...
  it("Can't redeem more than position size", async function () {
    await tokenCurrency.approve(pricelessPositionManager.options.address, toWei("1000"), { from: sponsor });
    await collateral.approve(pricelessPositionManager.options.address, toWei("1000"), { from: other });
    await collateral.approve(pricelessPositionManager.options.address, toWei("1000"), { from: sponsor });
  ...
  it("Sponsor can use repay to decrease their debt", async function () {
    ...
    const initialSponsorTokens = await tokenCurrency.methods.balanceOf(sponsor).call();
    ...
    const initialTotalTokensOutstanding = await pricelessPositionManager.methods.totalTokensOutstanding().call();
    ...
    const tokensPaid = initialSponsorTokens.sub(await tokenCurrency.methods.balanceOf(sponsor).call());
    ...
    const totalTokensOutstandingDecreased = initialTotalTokensOutstanding.sub(
      await pricelessPositionManager.methods.totalTokensOutstanding().send({ from: accounts[0] })
    );
    ...
    await tokenCurrency.approve(pricelessPositionManager.options.address, toWei("60"), { from: sponsor });
  ...
  it("Basic fees", async function () {
    ...
    const payTooManyFeesResult = await pricelessPositionManager.methods.payRegularFees().call();
  ...
  it("Gulps non-PfC collateral into PfC", async function () {
    ...
    await collateral.mint(pricelessPositionManager.options.address, toWei("0.5"), { from: collateralOwner });
  ...
  it("Final fees", async function () {
    ...
    // Determine the expected store balance by adding 1% of the sponsor balance to the starting store balance.
    const expectedOptimisticOracleBalance = (
    ...
    await tokenCurrency.methods
      .approve(pricelessPositionManager.options.address, sponsorInitialSynthetic)
      .send({ from: sponsor });
    await pricelessPositionManager.methods.settleExpired({ from: sponsor }).send({ from: accounts[0] });
    const sponsorFinalCollateral = await collateral.methods.balanceOf(sponsor).call();
    const sponsorFinalSynthetic = await tokenCurrency.methods.balanceOf(sponsor).call();
  ...
  describe("Precision loss is handled as expected", () => {
    beforeEach(async () => {
      ...
      let startTime = await pricelessPositionManager.methods.getCurrentTime().call();
      await pricelessPositionManager.setCurrentTime(startTime.addn(1));
    ...
    it("settleExpired() returns the same amount of collateral that totalPositionCollateral is decreased by", async () => {
      ...
      const tokenHolderInitialCollateral = await collateral.methods.balanceOf(tokenHolder).call();
      await pricelessPositionManager.methods.settleExpired({ from: tokenHolder }).send({ from: accounts[0] });
      const tokenHolderFinalCollateral = await collateral.methods.balanceOf(tokenHolder).call();
      const tokenHolderFinalSynthetic = await tokenCurrency.methods.balanceOf(tokenHolder).call();
    ...
    it("withdraw() returns the same amount of collateral that totalPositionCollateral is decreased by", async () => {
      ...
      await pricelessPositionManager.methods.withdrawPassedRequest({ from: sponsor }).send({ from: accounts[0] });
  ...
  it("Not enough collateral to pay final fees, reverts expire", async function () {
    ...
    // To settle positions the DVM needs to be to be queried to get the price at the settlement time.
    assert(
      await didContractThrow(pricelessPositionManager.methods.expire({ from: other }).send({ from: accounts[0] }))
    );
  ...
  it("Oracle swap post expiry", async function () {
    // Approvals
    await collateral.approve(pricelessPositionManager.options.address, toWei("100000"), { from: sponsor });
    await tokenCurrency.approve(pricelessPositionManager.options.address, toWei("100000"), { from: tokenHolder });
    await tokenCurrency.approve(pricelessPositionManager.options.address, toWei("100000"), { from: other });
    ...
    await newOptimisticOracle.methods
      ...
      .send({ from: proposer });
    await collateral.approve(newOptimisticOracle.options.address, toWei("1000000"), { from: proposer });
    ...
    await pricelessPositionManager.methods.settleExpired({ from: other }).send({ from: accounts[0] });
  ...
  it("Emergency shutdown: lifecycle", async function () {
    ...
    assert(
      await didContractThrow(
        pricelessPositionManager.methods.emergencyShutdown({ from: other }).send({ from: accounts[0] })
      )
    );
    ...
    assert.equal(eventResult[0].args.caller, financialContractsAdmin.options.address);
    ...
    assert(
      await didContractThrow(
        pricelessPositionManager.methods.settleExpired({ from: sponsor }).send({ from: accounts[0] })
      )
    );
    ...
    await proposeAndSettleOptimisticOraclePrice(
      priceFeedIdentifier,
      (await pricelessPositionManager.methods.expirationTimestamp().call()).toNumber(),
      toWei("1.1")
    );
  ...
  it("Non-standard ERC20 delimitation", async function () {
    ...
    await USDCToken.approve(customPricelessPositionManager.options.address, createCollateral, { from: sponsor });
    ...
    await USDCToken.approve(customPricelessPositionManager.options.address, depositCollateral, { from: sponsor });
    ...
    // To settle positions the DVM needs to be to be queried to get the price at the settlement time.
    await customPricelessPositionManager.methods.expire({ from: other }).send({ from: accounts[0] });

    // Push a settlement price into the mock oracle to simulate a DVM vote. Say settlement occurs at 1.2 Stock/USD for the price
    // feed. With 100 units of outstanding tokens this results in a token redemption value of: TRV = 100 * 1.2 = 120 USD.
    await collateral.approve(customPricelessPositionManager.options.address, toWei("1000000"), { from: proposer });
    const proposalTime = await optimisticOracle.methods.getCurrentTime().call();
    await optimisticOracle.proposePrice(
    ...
    await tokenCurrency.methods
      ...
      .send({ from: tokenHolder });
    let settleExpiredResult = await customPricelessPositionManager.methods.settleExpired({ from: tokenHolder }).call();
    ...
    await tokenCurrency.methods
      ...
      .send({ from: sponsor });
    await customPricelessPositionManager.methods.settleExpired({ from: sponsor }).send({ from: accounts[0] });
