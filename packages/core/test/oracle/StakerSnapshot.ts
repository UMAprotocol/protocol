...
describe("StakerSnapshot", function () {
  ...
  describe("Slashing: unrealizedSlash consideration", function () {
    it("Reward retrieval correctly factors in unrealizedSlash", async function () {
      ...
      assert.equal(await staker.methods.outstandingRewards(account1).call(), toWei("229")); // 160 + 69 = 229.
      assert.equal(await staker.methods.outstandingRewards(account2).call(), toWei("411")); // 480 - 69 = 411.
      ...
      await advanceTime(1500);
      assert.equal(await staker.methods.outstandingRewards(account1).call(), toWei("469")); // 160 + 69 + 960 * 1/4 = 469
      assert.equal(await staker.methods.outstandingRewards(account2).call(), toWei("1131")); // 480 - 69 + 960 * 3/4 = 1131
    ...
    it("Can correctly slash into staked balance of user", async function () {
      ...
      assert.equal(await staker.methods.outstandingRewards(account1).call(), toWei("660")); // 160 + 500 = 660
      assert.equal(await staker.methods.outstandingRewards(account2).call(), toWei("0")); // min(0, 480-500) = 0
      ...
      // Account 2 should receive their stake amount minus their slash amount of 20.
      assert.equal(
        await votingToken.methods.balanceOf(account2).call(),
        toBN(account2BalBefore)
          .add(amountToStake.muln(3).sub(toWei("20")))
          .toString()
      );
    });
  });
