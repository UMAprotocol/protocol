...
contract("MultiRole", function (accounts) {
  ...
  it("Events are emitted", async function () {
    ...
    // Add shared member.
    const addSharedResult = await multiRole.methods.addMember("2", account4).call({ from: account1 });
    ...
    const removeSharedResult = await multiRole.methods.removeMember("2", account4).call({ from: account1 });
    ...
    const renounceSharedResult = await multiRole.methods.renounceMembership("2").call({ from: account2 });
    ...
    const resetMemberResult = await multiRole.methods.resetMember("1", account2).call({ from: account1 });
