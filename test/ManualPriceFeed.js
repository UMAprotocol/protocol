...
contract("ManualPriceFeed", function(accounts) {
  ...
  it("No prices > One price > Updated price", async function() {
    ...
    assert(await didContractThrow(manualPriceFeed.latestPrice(symbolBytes)));
  ...
  it("Multiple symbols", async function() {
    ...
    assert(await didContractThrow(manualPriceFeed.latestPrice(firstSymbolBytes)));
    assert(await didContractThrow(manualPriceFeed.latestPrice(secondSymbolBytes)));
    assert(await didContractThrow(manualPriceFeed.latestPrice(absentSymbolBytes)));
  ...
  it("Non owner", async function() {
    ...
    assert(await didContractThrow(manualPriceFeed.pushLatestPrice(symbolBytes, 100, 500, { from: rando })));
  ...
  it("Push non-consecutive prices", async function() {
    ...
    assert(await didContractThrow(manualPriceFeed.pushLatestPrice(symbolBytes, 50, 500)));
  });
  ...
  it("Push a future price", async function() {
    ...
    assert(await didContractThrow(manualPriceFeed.pushLatestPrice(symbolBytes, currentTime + tolerance + 1, 500)));
