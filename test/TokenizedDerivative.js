...
contract("TokenizedDerivative", function(accounts) {
  ...
  testVariants.forEach(testVariant => {
    ...
    it(annotateTitle("AP Delegate Permissions"), async function() {
      ...
      await derivativeContract.approve(derivativeContract.address, web3.utils.toWei("1.1", "ether"), { from: apDelegate });
      await derivativeContract.redeemTokens({ from: apDelegate });

      assert(await didContractThrow(derivativeContract.withdraw(web3.utils.toWei("0.1", "ether"), { from: apDelegate })));
      assert(await didContractThrow(derivativeContract.deposit(await getMarginParams(web3.utils.toWei("0.1", "ether"), apDelegate))));
