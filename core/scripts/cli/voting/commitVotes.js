...
module.exports = async (web3, voting) => {
  ...
  if (roundPhase.toString() === VotePhasesEnum.REVEAL) {
    console.log(`The current vote phase is the "reveal" phase; in the reveal phase, you can only reveal already committed votes. You cannot vote on price requests in this phase.`);
  }
    ...
    for (let i = 0; i < filteredRequests.length; i++) {
        ...
        request.value = `${web3.utils.hexToUtf8(request.identifier)} @ ${style.formatSecondsToUtc(parseInt(request.time))}`
    }
    ...
    const checkbox = await inquirer.prompt({
      ...
      if (checkbox["requestsCheckbox"]) {
        console.log(style.bgGreen(`Next, enter prices for the selected requests. Prices must be positive numbers, invalid input will default to 0.`))
        ...
        if (newCommitments.length > 0) {
            ...
            // Print results
            console.log(style.bgGreen(`You have successfully committed ${successes.length} price${successes.length === 1 ? '' : 's'} in ${batches} batch${batches === 1 ? '' : 'es'}. (Failures = ${failures.length})`))
