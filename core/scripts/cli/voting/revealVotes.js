...
module.exports = async (web3, voting) => {
  ...
  if (roundPhase.toString() === VotePhasesEnum.COMMIT) {
    console.log(`The current vote phase is the "commit" phase; in the commit phase you can vote on pending price requests. You cannot reveal votes during this phase.`);
  }
    ...
    const checkbox = await inquirer.prompt({
      ...
      if (checkbox["requestsCheckbox"]) {        
        ...
        if (newReveals.length > 0) {
            ...
            // Print results
            console.log(style.bgGreen(`You have successfully revealed ${successes.length} price${successes.length === 1 ? '' : 's'} in ${batches} batch${batches === 1 ? '' : 'es'}. (Failures = ${failures.length})`))
