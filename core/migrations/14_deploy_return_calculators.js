...
module.exports = async function(deployer, network, accounts) {
  ...
  const { contract: levered2x } = await deploy(deployer, network, LeveragedReturnCalculator, 2, { from: keys.deployer });
  const { contract: levered1x } = await deploy(deployer, network, LeveragedReturnCalculator, 1, { from: keys.deployer });
