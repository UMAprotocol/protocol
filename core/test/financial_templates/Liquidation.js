...
contract("Liquidation", function(accounts) {
  ...
  const sponsorDisputeReward = sponsorDisputeRewardPct.div(toWei("1")).times(settlementTRV).dividedBy(toWei("1"));
  const disputerDisputeRewardPct = BN(toWei("0.05"));
  const disputerDisputeReward = disputerDisputeRewardPct.div(toWei("1")).times(settlementTRV).dividedBy(toWei("1"));
  const liquidationLiveness = BN(60).times(60).times(1); // In seconds
  ...
  describe("Liquidation has been created", () => {
      ...
      describe("Get a Liquidation", () => {
          it("Liquidation exists and params are set properly", async () => {
            ...
            assert.equal(newLiquidation.expiry.toString(), BN(startTime).plus(liquidationLiveness).toString());
      ...
      describe("Dispute a Liquidation", () => {
        ...
        it("Liquidation already expired", async () => {
            await liquidationContract.setCurrentTime(BN(startTime).plus(liquidationLiveness).toString());
            assert(await didContractThrow(
                liquidationContract.dispute(
                    liquidationParams.uuid,
                    sponsor,
