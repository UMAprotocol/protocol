...
contract("Testable", function() {
  it("isTest on", async function() {
    const testable = await TestableTest.new(true, Timer.address);
  ...
  it("isTest off", async function() {
    const testable = await TestableTest.new(false, Timer.address);
  ...
  it("In test environment, different Testable contracts reference the same Timer", async function() {
    const testable1 = await TestableTest.new(true, Timer.address);
    const testable2 = await TestableTest.new(true, Timer.address);
